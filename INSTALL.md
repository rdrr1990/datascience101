...
In this course we use `R` for our computations. `R` is a one-stop-shop for data science. 
 ...
 - Analyze many, many kinds of data: numeric, textual, audio, visual, DNA, social media... (`.csv`,`.xml`,`.pdf`, `.png`, `.sas`, `.dta`, `.json`, etc.)
...
If your operating system isn't new enough for the newest version of `R`, you should (at some point) consider upgrading your OS. However, the majority of the course material should work on a slightly older version like `3.3`, so you can use the link above to find the newest possible version for now. Talk to an instructor about any problems you encounter.
...
`RStudio` is an IDE (Integrated Development Environment). It helps with a number of things from keeping track of variables you've created and even helps you put your results into `PDF` documents or slides.
